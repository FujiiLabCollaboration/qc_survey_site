...
function vuePapers() {
  return new Vue({
    ...
    methods: {
      ...
      firstPapers(inputResponse) {
        document.getElementById("timenow").innerHTML = inputResponse.headers["last-modified"];
        this.importPapers(inputResponse.data);
    ...
    mounted(){
      axios.get('./input.tsv?' + Date.now()).then(response => this.firstPapers(response))
    },
  })
};
